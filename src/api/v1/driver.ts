...
import { verifyDriver, verifyDriverDocumentSchema, verifyDriverRegisterSchema } from "../../middlewares/driver";
...
app.post("/", verifyDriverRegisterSchema, async (req: Request, res: Response) => {
...
app.post("/documents", verifyDriver, verifyDriverDocumentSchema, async (req: Request, res: Response) => {
  try {
    ...
    if (AdhaarCard.id && DL.id) {
      ...
      if (!driverDocument) {
        return res.status(400).json({ message: "failed to upload driver document" });
      }
    }
    res.status(201).json({
